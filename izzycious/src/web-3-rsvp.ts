import { Address, ipfs, json, Value } from "@graphprotocol/graph-ts"
import {
  ConfirmedAttendee,
  DepositsPaidOut,
  NewEventCreated,
...
import { Account, Confirmation, RSVP, Event} from "../generated/schema";
...
export function handleDepositPaidOut(event: DepositsPaidOut): void{
  let thisEvent = Event.load(event.params.eventID.toHex());
  if (thisEvent) {
